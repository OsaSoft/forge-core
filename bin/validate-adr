    ...
    for key, prop in schema.get("properties", {}).items():
        ...
        if "maxLength" in prop and isinstance(value, str):
            if len(value) > prop["maxLength"]:
                problems.append(f"{key}: length {len(value)} > maxLength {prop['maxLength']}")
        ...
        if expected_type == "array" and isinstance(value, list):
            ...
            if prop.get("uniqueItems") and len(value) != len(set(value)):
                problems.append(f"{key}: duplicate items found")
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] == "--test":
        ...
        module_root = script_directory.parent
    ...
    if not HAS_YAML:
        print("  WARN  pyyaml not installed — skipping: nested YAML structure, date type coercion, multiline values. Install: pip install pyyaml")
