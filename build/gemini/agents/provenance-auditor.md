...
1. read_file `module.yaml` to identify the module name and version.
2. run_shell_command `forge provenance` on the target directory to gather current provenance state. For user-scope: `forge provenance ~/.claude`. For project-scope: `forge provenance .claude`.
...
6. read_file provenance sidecars in `build/` and compare `resolvedDependencies` digests against current source files -- flag any source-level staleness.
...
1. run_shell_command `forge install .` and capture output.
