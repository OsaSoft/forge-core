...
2. **Detect gitleaks**: run_shell_command `which gitleaks` to check availability. If present, use it for secret scanning. If absent, fall back to grep_search-based pattern matching.
...
run_shell_command these scan layers based on the determined mode:

**PII scan** (all modes):
- For each known PII term, search with grep_search (case-insensitive) across the scan scope
...
- If gitleaks unavailable: grep_search for common secret patterns (API keys, tokens, private keys, connection strings)
...
- **read_file-only** — never modify files, rewrite history, or force push. Report findings and provide commands for the user to execute.
- Use gitleaks when available, fall back to grep_search patterns when not.
