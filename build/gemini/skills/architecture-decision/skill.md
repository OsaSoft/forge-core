...
read_file `$ADR_PREFIX` (default: `number`):
...
3. When asked about a specific topic, search ADR titles and content for relevant keywords. read_file and summarize matching ADRs with: Context, Decision, Consequences, Status.
...
5. Check for overlap first. read_file every existing ADR in the target directory (titles AND bodies, not just filenames). For each existing ADR, evaluate the relationship:
...
6. Use the MADR light template. Fill in frontmatter (`status`, `date`) and sections. write_file to the ADR directory.

7. Set status to `Proposed` unless the decision is already confirmed — then set `Accepted`.

8. run_shell_command the Validate workflow on the new file.
