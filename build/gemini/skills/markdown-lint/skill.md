...
---
Format and lint a markdown document for consistent code formatting, proper list structure, heading hierarchy, and link hygiene. read_files a file, applies all rules, writes the corrected version after confirmation.

## Instructions

### Step 1: read_file the target file

If an argument was provided, use it as the file path. Otherwise, ask which file to lint.

Check TLP before reading:
- GREEN/CLEAR: read_file directly
- AMBER: Use `safe-read` via run_shell_command
- RED: Refuse
...
4. If web_fetch available and URL is public, fetch the page title
...
### Step 7: write_file the corrected file

- AMBER: use `safe-write write` via run_shell_command
- GREEN/CLEAR: use write_file tool directly
