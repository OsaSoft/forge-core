...
run_shell_command the companion script to show the current state of all inherited content.

1. run_shell_command `publish-prompts.sh --type all` and present the table to the user.
...
1. run_shell_command `publish-prompts.sh --type all` to get current state.
2. For each file in the manifest, compare the current upstream body SHA against the manifest SHA:
   ...
   - **Mismatch** → upstream has changed. Check local state:
     ...
     - **Adapted locally** → show 3-way context:
       - read_file the upstream file (current version)
       - read_file the local file (current version)
...
5. run_shell_command `make install` to regenerate the manifest with the new entry.
...
2. read_file the local file and the manifest to identify the upstream module.
...
8. run_shell_command `make install` to update the manifest.
...
2. run_shell_command the full provenance scan to identify existing inherited content.
...
5. run_shell_command `make install` to generate manifests for all tracked content.
