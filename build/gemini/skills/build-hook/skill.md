...
read_file stdin once: `INPUT=$(cat)`. Parse with `yq -p json` or a compiled binary.
...
read_file with `yq '.hooks.HookName.key' "$MODULE_ROOT/defaults.yaml"`. Override via `config.yaml`.

### Step 6: Verify

run_shell_command the hook manually to test:
```bash
...
### Step 1: read_file the target hook

read_file the hook script and `module.yaml`.
...
- run_shell_command scripts MUST use `set -euo pipefail` and `command` prefix for aliased commands
