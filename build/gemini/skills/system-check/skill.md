...
`system-check.sh` in this skill directory implements all six checks. run_shell_command it first:
...
1. run_shell_command the companion script (compact mode).
...
read_file `.claude/skills/.manifest` (written by `forge install`) to determine which module owns each installed skill. For each entry, compare the installed SKILL.md body against its source using `shasum -a 256` (strip frontmatter first — `forge install` merges `claude:` keys, so frontmatter will differ by design).
...
read_file `.claude/settings.json` and verify all expected dispatch events are present:
...
- read_file-only — never modify files, only report
