...
### Step 3: write_file the schema

Use the reference schema for the detected artifact type from the [conventions table](#field-requirements-by-artifact-type).

write_file `.mdschema` to the target directory using the write_file tool.

### Step 4: Validate

run_shell_command validation immediately to confirm the schema works:
