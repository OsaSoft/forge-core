...
1. read_file the target repo's `rules/` directory. Classify each file:
...
2. read_file the source rule set. Default source: the module that owns this skill (`forge-core/rules/`). The user can specify a different source.
...
3. For each source rule, decide relevance to the target:
   ...
   | Memory/tooling (MemoryFiles, KnownIssues, run_shell_commandAliases) | Port only if relevant to the target's stack |
...
6. Ask the user to confirm before writing. Apply changes with the write_file tool.
