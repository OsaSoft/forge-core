...
| ------------ | ------------------------------------------------ | -------------------------------- |
| **Analyze**  | "learn from this session", "extract learnings"   |(#analyze-the-session)  |
...
- **New rules**: write to `rules/` using the write_file tool
- **Updates**: use the replace tool on the target file
- **New skills/agents**: write to `skills/` or `agents/` using the write_file tool
