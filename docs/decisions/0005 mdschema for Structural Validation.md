...
Chosen option: **`.mdschema` files**. Each directory carries its own schema declaring required frontmatter, heading hierarchy (`no_skip_levels`), and max depth. Both the `validate-module` binary and the standalone `mdschema` CLI consume them.
